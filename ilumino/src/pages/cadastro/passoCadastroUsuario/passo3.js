...
import {StyleSheet, Text, ScrollView, Platform,TouchableOpacity, View, ImageBackground, TextInput, Alert} from 'react-native';
import Icon from 'react-native-vector-icons/FontAwesome';
import {Container, Header, Left, Body, Right, Content} from 'native-base';

export default class Passo3 extends Component {
  ...
  nextPass = ()=> {
    const {nome, sobreNome, cpf, nickname, celular, whatsapp } = this.props.navigation.state.params.data;
    const {email} = this.state;
    if(email.length === 0)
      Alert.alert('Erro - email vazio!');
    else
      this.props.navigation.navigate('Passo4',{data:{nome, sobreNome, cpf, nickname, celular, whatsapp, email}});
}
