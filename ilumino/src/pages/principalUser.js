...
import SiderBarUsuario from '../menu/siderBarUsuario';
...
mainPos = ()=> {
  ...
  async componentDidMount() {
    ...
    if (id) {
      ...
      if (response.ok) {
        const {sobreNome, cpf, nickname, celular, whatsapp, email} = response.data.data;

        this.props.getUser(obj={
          sobreNome, cpf, nickname, celular, whatsapp, email
        })
        this.setState({ imageSource: response.data.data.fotoPath, nomeUsuario: response.data.data.nome });

      } else {
        Alert.alert('Error - Por favor, verifique sua conexão com a internet');
        //this.setState({loading:false})
      }
    } else {
      Alert.alert('Error')
  ...
  render() {
    return (
      <Drawer ref={(ref) => { this.drawer = ref; }}
        content={<SiderBarUsuario navigator={this.navigator} navigation={this.props.navigation} />}
...
function mapDispathToProps(dispatch){
  return{
    getUser: (obj) => dispatch({type:'GET_USER', payload:obj}),
    logout: ()=> dispatch({type:'LOGOUT'})
  }
}

export default connect(mapStateToProps, mapDispathToProps) (PrincipalUser);
